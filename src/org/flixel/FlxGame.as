package org.flixel
{
	...
	public class FlxGame extends Sprite
	{
		...
		//protected function deviceAdd(FlashEvent:GameInputEvent):void {
		//	var myDevice:GameInputDevice = GameInput.getDeviceAt(0);//1st gamepad is "0" - more gamepads would be "1", "2", "3", etc.
				...
				if(useSoundHotKeys)
				{
					...
					switch(c)
					{
						...
						case 187:
							FlxG.mute = false;
				    		FlxG.volume = FlxG.volume + 0.1;
							if ( Registry.options && Registry.options.show ) {
		...
		protected function create(FlashEvent:Event):void
		{
			...
			//stage.addEventListener(GameInputEvent.DEVICE_ADDED, deviceAdd);
			addChild(_mouse);
